...
图片_BMP& 图片_BMP::构造(const 基础类::字符串 &图片文件位置) {
	...
	// 读取文件基本信息
	临时文件.读((i8*) &BMP文件头, sizeof(BMP_文件头));

	// 读取BMP相关信息
	临时文件.读((i8*) &BMP信息, sizeof(BMP_信息));
...
图片_BMP& 图片_BMP::复制构造(const 图片_BMP &其他实例) {
	throw 异常("图片_BMP:复制构造方法不存在！\n"); // 默认操作，实现复制构造方法需将其删除

	return (*this);
}

图片_BMP& 图片_BMP::移动构造(图片_BMP &&其他实例) {
	throw 异常("图片_BMP:移动构造方法不存在！\n"); // 默认操作，实现移动构造方法需将其删除
